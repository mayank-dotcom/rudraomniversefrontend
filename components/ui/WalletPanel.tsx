  ...
  return (
    <div className={`flex-1 ${isDarkMode ? "custom-scrollbar" : "light-scrollbar"} overflow-y-auto`}>
      <div className="p-6 space-y-8">
        ...
        {/* Balance Card */}
        <div className={`p-6 border-2 ${isDarkMode ? "bg-white/5 border-white/10" : "bg-black/5 border-black"}`}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className={`text-[8px] font-mono uppercase tracking-[0.3em] ${isDarkMode ? "text-white/40" : "text-black"}`}>
                Wallet Balance
              </p>
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            ...
            <Coins className={`h-4 w-4 ${isDarkMode ? "text-white/30" : "text-yellow-500"}`} style={!isDarkMode ? spinSlow : undefined} />
            <span className={`text-[10px] font-mono uppercase tracking-widest ${isDarkMode ? "text-white/30" : "text-black"}`}>
              coins
            </span>
          </div>
          <div className="flex gap-6 mt-4">
            <div>
              <p className={`text-[8px] font-mono uppercase tracking-[0.2em] ${isDarkMode ? "text-white/30" : "text-black"}`}>
                Earned
              </p>
              <p className={`text-sm font-bold ${isDarkMode ? "text-white/70" : "text-black"}`}>
        ...
        {!wallet?.referral_code && (
          <div className={`p-5 border-2 ${isDarkMode ? "bg-white/5 border-white/10" : "bg-black/5 border-black"}`}>
            <div className="flex items-center gap-2 mb-3">
              <Gift className={`h-3.5 w-3.5 ${isDarkMode ? "text-white/40" : "text-black"}`} />
              <p className={`text-[8px] font-mono uppercase tracking-[0.3em] ${isDarkMode ? "text-white/40" : "text-black"}`}>
                Have a referral code?
              </p>
            </div>
            <p className={`text-[11px] mb-4 ${isDarkMode ? "text-white/50" : "text-black"}`}>
            ...
            <div className="flex gap-2">
              <input
                ...
                className={`flex-1 px-3 py-2.5 text-[11px] font-mono uppercase tracking-widest border-2 ${isDarkMode ? "bg-transparent border-white/20 text-white placeholder:text-white/20" : "bg-transparent border-black text-black placeholder:text-black"} focus:outline-none focus:border-[#00DDDD] transition-colors`}
                maxLength={20}
              />
              <button
        ...
        {referralStats?.my_referral_code && (
          <div className={`p-5 border-2 ${isDarkMode ? "border-white/10" : "border-black"}`}>
            <div className="flex items-center gap-2 mb-3">
              <Users className={`h-3.5 w-3.5 ${isDarkMode ? "text-white/40" : "text-black"}`} />
              <p className={`text-[8px] font-mono uppercase tracking-[0.3em] ${isDarkMode ? "text-white/40" : "text-black"}`}>
                Your Referral Code
              </p>
            </div>
            ...
            <p className={`text-[10px] mt-3 ${isDarkMode ? "text-white/40" : "text-black"}`}>
        ...
        {wallet?.recent_transactions && wallet.recent_transactions.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-4">
              <TrendingUp className={`h-3.5 w-3.5 ${isDarkMode ? "text-white/40" : "text-black"}`} />
              <p className={`text-[8px] font-mono uppercase tracking-[0.3em] ${isDarkMode ? "text-white/40" : "text-black"}`}>
                Transaction History
              </p>
            </div>
            <div className="space-y-2">
              {wallet.recent_transactions.slice(0, 10).map((tx, i) => {
                ...
                return (
                  <div key={tx.id || i} className={`flex items-start gap-3 p-3 border-2 ${isDarkMode ? "bg-white/[0.02] border-white/10" : "bg-black/[0.02] border-black"}`}>
                    <div className={`h-7 w-7 flex items-center justify-center shrink-0 ${isDarkMode ? "bg-white/5" : "bg-black/5"}`}>
                      <Icon className={`h-3 w-3 ${isDarkMode ? "text-white/40" : "text-black"}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`text-[10px] font-medium truncate ${isDarkMode ? "text-white/70" : "text-black"}`}>
                        {tx.description}
                      </p>
                      <p className={`text-[8px] font-mono mt-0.5 ${isDarkMode ? "text-white/30" : "text-black/60"}`}>
