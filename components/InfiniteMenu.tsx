  ...
  public update(deltaTime: number, targetFrameDuration = 16): void {
    ...
    if (this.isPointerDown) {
      ...
      if (this.isHovered) {
        ...
        // Slow majestic auto-rotation when not hovered
...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    this.camera.position[2] = 3.0 * scale;
    this.init(onInit);
  }
  ...
  private animate(deltaTime: number): void {
    ...
    // Update hoverProgress smoothly
    const timeScale = deltaTime / this.TARGET_FRAME_DURATION + 0.0001;
    const targetHover = this.control.isHovered ? 1.0 : 0.0;
    const lerpSpeed = 0.08 * timeScale;
    this.hoverProgress += (targetHover - this.hoverProgress) * lerpSpeed;
    ...
    positions.forEach((p, ndx) => {
      ...
      if (ndx === nearestVertexIndex) {
        // Smoothly scale the active focused circle to fill the screen
        const zoomedScale = 0.15;
        finalScale = finalScale * (1 - this.hoverProgress) + zoomedScale * this.hoverProgress;
      } else {
        // Smoothly fade out the other circles
        finalScale = finalScale * (1 - this.hoverProgress);
      }
  ...
  private onControlUpdate(deltaTime: number): void {
    const timeScale = deltaTime / this.TARGET_FRAME_DURATION + 0.0001;
    const damping = 5 / timeScale;
    const cameraTargetZ = 3.0 * this.scaleFactor; // Constant scale, no zoom-in or zoom-out!
    ...
    // Always update active item, even during auto-rotation
    const nearestVertexIndex = this.findNearestVertexIndex();
    const itemIndex = nearestVertexIndex % Math.max(1, this.items.length);
    this.onActiveItemChange(itemIndex);

    if (!this.control.isPointerDown) {
      if (this.control.isHovered) {
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <div
          ...
          `}
        >
          <h2 className="font-serif font-light text-neutral-900 text-3xl md:text-5xl italic tracking-tight">
            {activeItem.title}
          </h2>
          <p className="font-mono text-[10px] md:text-xs uppercase tracking-[0.25em] text-neutral-500">
